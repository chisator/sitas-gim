...
export function ActivitiesCarousel() {
    ...
    const handleReserve = async (classId: string, activityTitle: string, classTime: string) => {
        ...
        // If RPC fails or doesn't exist, we might need manual update. 
        // But for now let's assume simple update if RPC not available.
        if (creditError) {
            // Try manual update
            const { error: manualUpdateError } = await supabase
                .from('profiles')
                .update({ reservation_credits: userCredits - 1 })
                .eq('id', userId)

            if (manualUpdateError) {
                console.error("Error reducing credits", manualUpdateError)
                // technically significant issue if reservation made but credits not reduced
            }
        }
    }
    ...
    const handleCancelReservation = async (classId: string, activityTitle: string, classTime: string) => {
        ...
        if (cancelError) {
            ...
            setUserCredits(prev => prev - 1)
            setReservationCounts(prev => ({ ...prev, [classId]: (prev[classId] || 0) + 1 }))
            toast.error("Error al cancelar", { description: cancelError.message })
            return
        }

        // Refund credit
        const { error: refundError } = await supabase.rpc('increment_credits', { user_id: userId })
