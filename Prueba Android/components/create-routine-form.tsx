...
export function CreateRoutineForm({ athletes, creatorId, trainers = [], isAdmin = false, exerciseCatalog = [] }: CreateRoutineFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const trainer_id = isAdmin ? selectedTrainerId : creatorId
      if (!trainer_id) throw new Error("Debe seleccionarse un entrenador")
      ...
      const { data: inserted, error: insertErr } = await supabase
        ...
        .insert({
          ...
          description,
          trainer_id,
          start_date: new Date(`${startDate}T12:00:00Z`).toISOString(),
          end_date: new Date(`${endDate}T12:00:00Z`).toISOString(),
