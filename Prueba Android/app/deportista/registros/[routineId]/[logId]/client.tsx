...
export default function EditLogPage({ params }: { params: { routineId: string; logId: string } }) {
    ...
    useEffect(() => {
        async function loadData() {
            ...
            if (logData && logData.entries) {
                logData.entries.sort((a: any, b: any) => (a.order || 0) - (b.order || 0))
            }
        ...
        loadData()
    }, [params.routineId, params.logId, router, supabase])
    ...
    if (!log) return <div className="p-8 text-center bg-transparent text-gray-900 border-none dark:text-gray-100">Registro no encontrado</div>
